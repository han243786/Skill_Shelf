...
本文提出“拓扑式模块化治理”模型，将软件项目表示为一张朝北极星定向生长的可审计白箱网络：北极星定义最终状态前提，功能树/全量树记录能力与文件事实，模块树/拓扑树记录节点、父级和通信边，工作模式是改变拓扑的操作类型，QPCursor 是代理接管坐标，治理热度决定证据强度，局部不变量把防回退规则绑定到具体模块，closeout 是拓扑一致性证明。该模型通过北极星对齐、白箱节点、父级路由、核心矩阵同步、工作模式入口卡、G0-G5 热度触发、局部不变量、旧路径债隔离、冷文档降级和 AI 事实锚定，解决行业中长期存在的结构腐化、架构侵蚀、技术债滚雪球、跨模块耦合、文档漂移、AI 生成速度超过治理能力等痛点。
...
拓扑式治理；北极星；定向生长；模块化；白箱网络；功能树；模块树；QPCursor；架构侵蚀；技术债；AI 协作开发；父子通信；closeout；软件演化
...
4. 它处于哪一个 QPCursor、治理热度和局部不变量约束下？
5. 它是否同步了北极星对齐、功能树/全量树、模块树/拓扑树、代码、契约、测试和旧路径状态？
6. 它的 closeout 是行为通过，还是拓扑一致？
...
拓扑式治理的回应：不替代敏捷，而是在每个任务入口和 closeout 增加北极星对齐、节点、边、旧路径、核心矩阵同步和门禁证据。
...
拓扑式治理的回应：承认 bounded context 的价值，但把治理范围扩展到北极星、功能树/全量树、模块树/拓扑树、代码、测试、文档、脚本和工作模式。
...
拓扑式治理的回应：把技术债拆成旧路径债、模块 owner 债、public surface 债、横向连接债、北极星/功能树/拓扑树漂移债和模式污染债，并要求单独批次清理。
...
| Hexagonal Architecture | ports/adapters 隔离外部设备和内部应用 | 不管理核心矩阵、旧路径、AI 幻觉 | 把 port 作为拓扑边并要求 closeout |
...
```text
NorthStar: final_state -> convergence_signal
```

功能树/全量树：

```text
FullFeatureTree: feature_or_file -> role_and_protected_behavior
```

模块树/拓扑树：

```text
ModuleTopologyTree: module_node -> whitebox(module_node)
```

同步约束：

```text
meaningful_change(task) => confirm(NorthStar_alignment)
file_or_feature_change(item) => update_or_confirm(FullFeatureTree)
module_boundary_change(module_node) => update_or_confirm(ModuleTopologyTree)
```
...
QPCursor 是当前代理接管坐标。它必须记录 repo baseline、mode stack、North Star alignment、growth vector、scope、topology slice、governance heat、required depth、local invariants、interface freeze、allowed workset、next action、stop rules 和 evidence。
...
局部不变量把北极星、功能树/全量树和模块树/拓扑树投影到模块、边、切面、public surface 或门禁。它必须在编辑模块前加载，并在 closeout 中说明是否满足。
...
| 类型 | 例子 | 维护规则 |
| --- | --- | --- |
| 热资产 | 北极星、功能树/全量树、模块树/拓扑树、当前契约、工作模式路由、局部防回退护栏 | 随变更同步 |
...
功能树/全量树和模块树/拓扑树可能增长到数千行。若搜索定位、路径反查和门禁仍然稳定，文件体量本身不是拆分理由。拆分只有在事实证明定位效率、门禁稳定性、路径反查可靠性或合并冲突显著受损时才成立。
...
| 微服务边界与文档事实漂移 | Microservices | 核心矩阵同步、旧路径债隔离 |
...
只要搜索和门禁稳定，大型功能树/全量树和模块树/拓扑树不拆分也不应显著影响定位效率。
...
拓扑式治理不是让 AI 说得更漂亮，而是限制 AI 只能在北极星、真实节点、真实边和真实门禁内推进。提示词只是入口，真正有效的是模板、脚本、核心矩阵和 closeout。
...
4. `templates/`：北极星、功能树、项目拓扑、工作模式、任务卡、节点卡、QPCursor、heat、local invariants、closeout、AI 启动提示词。
5. `schemas/`：北极星、功能树、QPCursor、任务卡、节点卡、closeout、heat、local invariants 的机器可读协议。
6. `scripts/`：只读盘点、脚手架安装、骨架检查、QPCursor 检查、cursor/topology 检查、ledger 检查和 sibling-edge 例外检查。
...
| L4 Operational | 接入具体项目 CI、真实门禁、核心文档矩阵和案例证据 |
