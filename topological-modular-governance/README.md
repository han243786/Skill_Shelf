...
这是一套给大型项目使用的拓扑式模块化治理方案。它把项目看成一个朝北极星定向生长的白箱网络：北极星定义最终状态前提，功能树/全量树记录能力与文件事实，模块树/拓扑树记录节点、父级和通信边，三种开发模式加一种维护模式共同定义网络变更入口。

当前版本已经升级为 North-Star-first 治理包：每次治理任务默认经过 `北极星 -> 功能树/全量树 -> 模块树/拓扑树 -> QPCursor -> G0-G5 治理热度 -> 局部防回退护栏 -> allowed workset -> evidence -> cursor update`。
...
- 需要同时管理北极星、功能树/全量树、模块树/拓扑树、契约、测试、文档和 closeout 的项目。
...
再建立或更新 QPCursor：声明北极星对齐、生长向量、治理热度 G0-G5、Light/Standard/Precision 强度、局部防回退护栏、接口冻结、allowed workset、下一步动作和 evidence。
定位本轮影响的父模块、子模块、public 方法、接口、测试、文档和治理资产。
开发态禁止子模块横向直连；所有跨子模块通信先走父级、契约、adapter、事件或登记接口。
每次 closeout 证明拓扑一致：北极星对齐、功能树/全量树、模块树/拓扑树、代码路径、测试、契约和旧路径状态一致。
```
...
| 热度 | 最低强度 | 典型场景 |
| --- | --- | --- |
| G0-G1 | Light | 叶子级或模块内部私有清理，无接口、状态、持久化、安全或运行时语义变化 |
...
热度只能被证据升降，不能因为 diff 小、测试暂时通过、或者“只是文档”而降档。
...
- `references/08-north-star-directional-growth.md`: 北极星定向生长、QPCursor 和 G0-G5 热度运行模型。
- `references/09-local-invariants.md`: 局部不变量的绑定对象、格式和 closeout 关系。
- `templates/NORTH_STAR.md`: 北极星最终状态与定向生长模板。
- `templates/FULL_FEATURE_TREE.md`: 功能树/全量树模板。
...
- `templates/QPCURSOR.md`: QPCursor 协议模板。
- `templates/GOVERNANCE_HEAT.md`: G0-G5 治理热度矩阵。
- `templates/LOCAL_INVARIANTS.md`: 模块级局部不变量模板。
...
- `scripts/check-qpcursor-governance.ps1`: 检查北极星、功能树、QPCursor、治理热度、局部防回退护栏和 evidence。
...
| 包 | 重点 |
| --- | --- |
| `heavy-scale-exploitation-governance-1` | 旧式重型治理参考，包含重流程、矩阵和门禁；不再作为本包默认地基 |
| `modular-refactor` | 具体执行模块抽离、整理和等价重构 |
| `topological-modular-governance` | 用北极星、功能树和拓扑树组织定向生长，规定节点、边、工作模式和一致性 closeout |

推荐使用方式：

```text
topological-modular-governance
  -> 建立北极星、功能树、拓扑树和 QPCursor
...
这个包默认达到 `L3 Product-Like`，并内置 L4 hardening 门禁。具体项目只有在接入自己的 CI、真实测试、北极星、功能树/全量树、模块树/拓扑树和案例证据后才算 `L4 Operational`：
...
| L4 Operational | 接入具体项目 CI、真实门禁、核心文档矩阵和案例证据 | 包已提供 hardening 检查；仍需要在目标项目内落地 |
...
5. 新旧拓扑怎么共存、切换和裁剪？
6. closeout 后北极星、功能树/全量树、模块树/拓扑树、代码、测试、文档是否仍指向同一个事实？
